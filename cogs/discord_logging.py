...
class Logging:
	...
	async def write_log(self, log_type, log_content, attachment=None):
		...
		self.cur.execute('INSERT INTO log VALUES (?, ?, ?, ?)', (log_id, log_type, log_message.id, log_content,))
		self.con.commit()
...
class LoggingCog(commands.Cog):
	...
	async def on_voice_state_update(self, member, before, after):
		if before.channel != after.channel and after.channel is not None:
			log_content = f'{member} присоединился к каналу {after.channel.name} [<#{after.channel.id}>].'

			await Logging(self.bot, self.con).write_log(3, log_content)
			return
		for before_param in before:
			print(before_param.index(before))
